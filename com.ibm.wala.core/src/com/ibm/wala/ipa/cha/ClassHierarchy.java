...
//  //added by nameng
//  private BypassSyntheticClassLoader syntheticLoader;
  ...
  public boolean addClass(IClass klass) {
    ...
    Collection loadedSuperInterfaces = null;//the initialization is done by nameng
   ...
   // a little cleanup
      ...
      } else { //source code --added by nameng 
        map.put(klass.getReference(), findOrCreateNode(klass));
        return true;
      }
    ...
    try {      
      ...
      if (klass instanceof ShrikeClass) {
        if (DEBUG) {
          System.err.println(("Exception.  Clearing " + klass));
          Warnings.add(ClassExclusion.create(klass.getReference(), e.getMessage())); //moved by nameng
          return false;         //moved by nameng
        }
      } else { //added by nameng, so that the partial parsed source code can still be added to the class hierarchy
        // ignore --loadedSuperInterfaces is null        
      }
    }
