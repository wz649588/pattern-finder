...
//    for(Selector s:declaredMethods.keySet()){
...
//    }
	  ...
	  }catch (Exception e) {//added by nameng, to handle the case when getSuperclass() throws exceptions
	    return null;
	  }
  }
  ...
  public IField getField(Atom name) {
    ...
    }catch (Exception e) { //added by nameng to bypass exception thrown by getSuperclasses
      return null;
    }
  }
  ...
  public Collection<IMethod> getAllMethods() {
    ...
    try { //added by nameng to get all methods of the current class
      if (getSuperclass() != null) {
        result.addAll(getSuperclass().getAllMethods());
      }
    } catch(Exception e) {
      // do nothing added by nameng, so that even though super class is not resolved, we can still get all methods of the current class
    }
    return result;
  }
