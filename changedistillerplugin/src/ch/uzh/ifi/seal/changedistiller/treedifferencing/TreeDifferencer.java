...
import ch.uzh.ifi.seal.changedistiller.treedifferencing.matching.BestLeafTreeMatcher;
...
import edu.vt.cs.append.EnhancedBestLeafMatchDifferencer;
...
public class TreeDifferencer {
    ...
    public void calculateEditScript(Node left, Node right) {
        ...
        //modified by nameng
        TreeMatcher dnm = null;        
        if(left.getEntity().getLabel().equals("CLASS")) {
        	dnm = MatchingFactory.getMatcher(fMatch);
        } else {
          if (FileDistiller.checkIgnoreComments()) {
        	dnm = MatchingFactory.getMatcher2(fMatch);
          } else {
        	dnm = MatchingFactory.getMatcher(fMatch);
          }
        }
    ...
    /**
     * Ye Wang since 06/05/2017, to get tree matches
     * 
     * @param left
     * @param right
     * @return
     */
    public static HashSet<NodePair> getTreeMatches(Node left, Node right) {
    	HashSet<NodePair> match = new HashSet<>();
    	TreeMatcher dnm = null;
    	if (left.getEntity().getLabel().equals("METHOD")) {
			dnm = MatchingFactory.getMatcher(match);
			BestLeafTreeMatcher matcher = (BestLeafTreeMatcher) dnm;
			matcher.match2(left, right);
    	}
    	return match;
    }
    ...
    private void editScript(Node left, Node right) {
        ...
        while (breathFirst.hasMoreElements()) {
            ...
            if (fRightToLeftMatchPrime.get(x) == null) {
                ...
                //modified by nameng: If an UPDATE and a MOVE will be generated, I prefer an INSERT and a DELETE are created
                /*
                if (!equals && !matchContains(v, y, fMatchPrime)) {
                	fRightToLeftMatchPrime.remove(x);
                	fLeftToRightMatchPrime.remove(w);
                ...
                // iii. If (y, v) not in M'
                else if (!matchContains(v, y, fMatchPrime)) {
