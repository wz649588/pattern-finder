    ...
    private String getMethodSignature(AbstractMethodDeclaration methodDeclaration) {
        ...
        if (methodDeclaration.arguments != null) {
            for (int i = 0; i < methodDeclaration.arguments.length; i++) {
                ...
                // added by Ye Wang [
                if (methodDeclaration.arguments[i].type instanceof ParameterizedSingleTypeReference) {
                	ParameterizedSingleTypeReference pType = (ParameterizedSingleTypeReference) methodDeclaration.arguments[i].type;
                	signature.append(pType.token);
                	signature.append("<");
                	int length = pType.typeArguments.length;
                	if (length > 0) {
                		int max = length - 1;
                		for (int j = 0; j < max; j++) {
                			pType.typeArguments[j].print(0, signature);
                			signature.append(",");
                		}
                		pType.typeArguments[max].print(0, signature);
                	}
                	signature.append(">");
                } else
                // ]
