 ...
 * 
 * The source code originally comes from ASTParser and is also copyrighted IBM 
 * and others, EPL License.
 *******************************************************************************/
...
/**
 * <p>
 * This is a modification of the original ASTParser. It essentially continues to parse a Java source
 * file and to resolve bindings even when a parsing error is encountered.
 * </p>
 * <p>
 * Parsing errors often occur when encountering Java 5 syntax and unknown types. For example @Hello
 * if hello is not defined will produce a parsing error.
 * </p>
 */
public class PPAASTParser {
	 ...
	 *            the API level; one of the LEVEL constants declared on <code>AST</code>
	 */
	public PPAASTParser(int level) {
//		if ((level != AST.JLS2_INTERNAL) && (level != AST.JLS3)) {
...
//		}
	...
	private List getClasspath() throws IllegalStateException {
		Main main = new Main(new PrintWriter(System.out), new PrintWriter(System.err),
		...
		try {
			...
			if (this.sourcepaths != null) {
				for (int i = 0, max = this.sourcepaths.length; i < max; i++) {
					String encoding = this.sourcepathsEncodings == null ? null
							: this.sourcepathsEncodings[i];
					main.processPathEntries(Main.DEFAULT_SIZE_CLASSPATH, allClasspaths,
							this.sourcepaths[i], encoding, true, false);
				}
			}
			if (this.classpaths != null) {
				for (int i = 0, max = this.classpaths.length; i < max; i++) {
					main.processPathEntries(Main.DEFAULT_SIZE_CLASSPATH, allClasspaths,
							this.classpaths[i], null, false, false);
				}
			}
	 ...
	 * 
	 * @param enabled
	 *            <code>true</code> if incomplete bindings are expected, and <code>false</code> if
	 ...
	 * 
	 * <p>
	 * The user has to make sure that all the required types are included either in the classpath or
	 * source paths. All the paths containing binary types must be included in the
	 * <code>classpathEntries</code> whereas all paths containing source types must be included in
	 * the <code>sourcepathEntries</code>.
	 * </p>
	 * <p>
	 * All paths in the <code>classpathEntries</code> and <code>sourcepathEntries</code> are
	 * absolute paths.
	 * </p>
	 * <p>
	 * If the source paths contain units using a specific encoding (other than the platform
	 * encoding), then the given <code>encodings</code> must be set. When the <code>encodings</code>
	 * is set to non <code>null</code>, its length must match the length of
	 * <code>sourcepathEntries</code> or an IllegalArgumentException will be thrown.
	 * </p>
	 * <p>
	 ...
	 * @param encodings
	 *            the encodings of the corresponding sourcepath entries or <code>null</code> if the
	 *            platform encoding can be used.
	 * @param includeRunningVMBootclasspath
	 *            <code>true</code> if the bootclasspath of the running VM must be prepended to the
	 *            given classpath and <code>false</code> if the bootclasspath of the running VM
	 *            should be ignored.
	 ...
	 * 
	 * @param options
	 *            the table of options (key type: <code>String</code>; value type:
	 ...
	 * 
	 * @param enabled
	 *            <code>true</code> if bindings are wanted, and <code>false</code> if bindings are
	 *            not of interest
	 */
	...
	public void setKind(int kind) {
		if ((kind != K_COMPILATION_UNIT) && (kind != K_CLASS_BODY_DECLARATIONS)
				&& (kind != K_EXPRESSION) && (kind != K_STATEMENTS)) {
			throw new IllegalArgumentException();
		}
		this.astKind = kind;
	}
	 ...
	 * {@link #setCompilerOptions(Map)} as 1.5 code will not be properly parsed without setting the
	 * appropriate values for the compiler options: {@link JavaCore#COMPILER_SOURCE},
	 * {@link JavaCore#COMPILER_CODEGEN_TARGET_PLATFORM}, and {@link JavaCore#COMPILER_COMPLIANCE}.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param source
	 *            the Java model compilation unit whose source code is to be parsed, or
	 ...
	 * 
	 * @param source
	 *            the Java model class file whose corresponding source code is to be parsed, or
	 *            <code>null</code> if none
	 */
	 ...
	 * 
	 * @param source
	 *            the Java model compilation unit or class file whose corresponding source code is
	 ...
	 * @param length
	 *            the number of characters to parse, or -1 if the remainder of the source string is
	 *            to be parsed
	 */
	 ...
	 * 
	 * @param enabled
	 *            <code>true</code> if statements containing syntax errors are wanted, and
	 ...
	 * 
	 * @param owner
	 *            the owner of working copies that take precedence over underlying compilation
	 ...
	 * 
	 * @param unitName
	 *            the name of the compilation unit that would contain the source string, or
	 *            <code>null</code> if none
	 */
	 ...
	 * 
	 * @param project
	 *            the Java project used to resolve names, or <code>null</code> if none
	 */
	 ...
	 *                if the settings provided are insufficient, contradictory, or otherwise
	 *                unsupported
	 */
	public ASTNode createAST(boolean cleanup, IProgressMonitor monitor) {
		ASTNode result = null;
		if (monitor != null)
		...
		try {
			...
			result = internalCreateAST(cleanup, monitor);
		} finally {
	 ...
	 *            the compilation units to create ASTs for
	 * @param bindingKeys
	 *            the binding keys to create bindings for
	 * @param requestor
	 *            the AST requestor that collects abstract syntax trees and bindings
	 * @param monitor
	 *            the progress monitor used to report progress and request cancellation, or
	 *            <code>null</code> if none
	 * @exception IllegalStateException
	 ...
	 * @since 3.1
	 */
	public void createASTs(ICompilationUnit[] compilationUnits, String[] bindingKeys,
			ASTRequestor requestor, IProgressMonitor monitor) {
		try {
			int flags = 0;
			...
			if ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0) {
				...
				CompilationUnitResolver.resolve(compilationUnits, bindingKeys, requestor,
						this.apiLevel, this.compilerOptions, this.project, this.workingCopyOwner,
						flags, monitor);
			} else {
				CompilationUnitResolver.parse(compilationUnits, requestor, this.apiLevel,
	...
	public void createASTs(String[] sourceFilePaths, String[] encodings, String[] bindingKeys,
		...
		try {
			...
			if ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0) {
				...
				CompilationUnitResolver.resolve(sourceFilePaths, encodings, bindingKeys, requestor,
						this.apiLevel, this.compilerOptions, getClasspath(), flags, monitor);
			} else {
	 ...
	 * 
	 * @param elements
	 *            the Java elements to create bindings for
	...
	public IBinding[] createBindings(IJavaElement[] elements, IProgressMonitor monitor) {
		try {
			...
			return CompilationUnitResolver.resolve(elements, this.apiLevel, this.compilerOptions,
					this.project, this.workingCopyOwner, flags, monitor);
		} finally {
			// reset to defaults to allow reuse (and avoid leaking)
			initializeDefaults();
		}
	...
	private ASTNode internalCreateAST(boolean cleanup, IProgressMonitor monitor) {
		...
		case K_COMPILATION_UNIT:
			...
			try {
				...
				if (this.typeRoot instanceof ICompilationUnit) {
					/*
					 * this.compilationUnitSource is an instance of
					 * org.eclipse.jdt.internal.core.CompilationUnit that implements both
					 * org.eclipse.jdt.core.ICompilationUnit and
					...
					/*
					 * use a BasicCompilation that caches the source instead of using the
					 * compilationUnitSource directly (if it is a working copy, the source can
					 * change between the parse and the AST convertion) (see
					 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=75632)
					 */
					sourceUnit = new BasicCompilationUnit(sourceUnit.getContents(),
							sourceUnit.getPackageName(), new String(sourceUnit.getFileName()),
							this.project);
					...
					try {
						...
						PackageFragment packageFragment = (PackageFragment) this.typeRoot
								.getParent();
						BinaryType type = (BinaryType) this.typeRoot.findPrimaryType();
						IBinaryType binaryType = (IBinaryType) type.getElementInfo();
						// file name is used to recreate the Java element, so it has to be the
						// toplevel .class file name
						...
						sourceUnit = new BasicCompilationUnit(sourceString.toCharArray(),
								Util.toCharArrays(packageFragment.names), new String(fileName),
								this.project);
					} catch (JavaModelException e) {
						// an error occured accessing the java element
						StringWriter stringWriter = new StringWriter();
					...
					needToResolveBindings = ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0)
							&& this.unitName != null
							&& (this.project != null || this.classpaths != null
									|| this.sourcepaths != null || ((this.bits & CompilationUnitResolver.INCLUDE_RUNNING_VM_BOOTCLASSPATH) != 0))
				...
				if (needToResolveBindings) {
					...
					try {
						// parse and resolve
						// XXX BART's EDIT
						compilationUnitDeclaration = PPACompilationUnitResolver.resolve(sourceUnit,
								this.project, getClasspath(), searcher, this.compilerOptions,
								this.workingCopyOwner, flags, monitor);
					} catch (JavaModelException e) {
						flags &= ~ICompilationUnit.ENABLE_BINDINGS_RECOVERY;
						// XXX BART's EDIT
						compilationUnitDeclaration = PPACompilationUnitResolver.parse(sourceUnit,
								searcher, this.compilerOptions, flags);
				...
				// XXX BART's EDIT
				CompilationUnit result = PPACompilationUnitResolver.convert(
						compilationUnitDeclaration, sourceUnit.getContents(), this.apiLevel,
						this.compilerOptions, needToResolveBindings, wcOwner,
				...
				if (compilationUnitDeclaration != null
						&& ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0) && cleanup) {
	...
	private ASTNode internalCreateASTForKind() {
		...
		case K_STATEMENTS:
			ConstructorDeclaration constructorDeclaration = codeSnippetParsingUtil.parseStatements(
					this.rawSource, this.sourceOffset, this.sourceLength, this.compilerOptions,
					true, (this.bits & CompilationUnitResolver.STATEMENT_RECOVERY) != 0);
			...
			if (statements != null) {
				...
				for (int i = 0; i < statementsLength; i++) {
					if (statements[i] instanceof org.eclipse.jdt.internal.compiler.ast.LocalDeclaration) {
						converter.checkAndAddMultipleLocalDeclaration(statements, i,
								block.statements());
					} else {
			...
			return block;
		case K_EXPRESSION:
			org.eclipse.jdt.internal.compiler.ast.Expression expression = codeSnippetParsingUtil
					.parseExpression(this.rawSource, this.sourceOffset, this.sourceLength,
		...
		case K_CLASS_BODY_DECLARATIONS:
			final org.eclipse.jdt.internal.compiler.ast.ASTNode[] nodes = codeSnippetParsingUtil
					.parseClassBodyDeclarations(this.rawSource, this.sourceOffset,
			...
			if (nodes != null) {
				...
				typeDeclaration.setSourceRange(this.sourceOffset, this.sourceOffset
						+ this.sourceLength);
				rootNodeToCompilationUnit(typeDeclaration.getAST(), compilationUnit,
						typeDeclaration, codeSnippetParsingUtil.recordedParsingInformation, null);
				ast.setDefaultNodeFlag(0);
				ast.setOriginalModificationCount(ast.modificationCount());
	...
	private void propagateErrors(ASTNode astNode, CategorizedProblem[] problems,
