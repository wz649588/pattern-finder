...
public class JavaCAst2IRTranslator extends AstTranslator {
  ...
  // called for them
  @Override
  protected void declareFunction(CAstEntity N, WalkContext definingContext, SymbolTable symtab) {
    ...
    ((JavaSourceLoaderImpl) loader).defineAbstractFunction(N, owner, symtab);
  }
  ...
  protected void doCall(WalkContext context, CAstNode call, int result, int exception, CAstNode name, int receiver, int[] arguments) {
    ...
    if (!isStatic) {
      realArgs[0] = receiver;
      System.arraycopy(arguments, 0, realArgs,
           1, arguments.length);
    }
...
//  @Override
//  public boolean visitNode(CAstNode n, WalkContext context, CAstVisitor<WalkContext> visitor) {
//    return false;
//  }
//  
//  @Override 
