...
public class AstJavaSSAPropagationCallGraphBuilder extends AstSSAPropagationCallGraphBuilder {
  ...
  public static class EnclosingObjectReferenceKey extends AbstractFieldPointerKey {
    ...
    @Override
    public int hashCode() {
      //added by nameng because outer can be null
      if (outer == null) {
    ...
    public boolean equals(Object o) {
      //modified by nameng because outer can be null
      if (!(o instanceof EnclosingObjectReferenceKey))
        return false;
      EnclosingObjectReferenceKey key = (EnclosingObjectReferenceKey)o;
      if (key.outer == null) {
        if (outer != null)
          return false;
      } else if (!key.outer.equals(this.outer))
        return false;
      if (!key.getInstanceKey().equals(getInstanceKey()))
        return false;
      return true;
