...
//      System.out.print("");
      for (Iterator iter = superTypeNames.iterator(); iter.hasNext();) {
        ...
        SetOfClasses exclusions = getClassHierarchy().getScope().getExclusions();        
        if (domoType == null && exclusions != null && exclusions.contains(name.toString().substring(1))){
...
//commented by nameng, because due to partial program analysis, the super class may not be found
...
//      }
    ...
    public Collection<IClass> getDirectInterfaces() {
      ...
      for (Iterator iter = superTypeNames.iterator(); iter.hasNext();) {
        ...
        // Added by Ye Wang to debug
        IClassHierarchy classHierarchy = getClassHierarchy();
        com.ibm.wala.ipa.callgraph.AnalysisScope scope = classHierarchy.getScope();
        SetOfClasses exclusions = scope.getExclusions();
        String nameString = name.toString();
        String nameSubstring = nameString.substring(1);
        boolean isExclusion = exclusions != null && exclusions.contains(nameSubstring);
        
        
        // if (domoType == null && !getClassHierarchy().getScope().getExclusions().contains(name.toString().substring(1))){
        if (domoType == null && !isExclusion){
    ...
    private void addMethod(CAstEntity methodEntity, IClass owner, SymbolTable symtab) {
      declaredMethods.put(Util.methodEntityToSelector(methodEntity), new AbstractJavaMethod(methodEntity, owner, symtab));
    }
  ...
  private abstract class JavaEntityMethod extends AstMethod {
    ...
    public JavaEntityMethod(CAstEntity methodEntity, IClass owner, SymbolTable symtab) {
      super(owner, methodEntity.getQualifiers(), MethodReference.findOrCreate(owner.getReference(), Util
          .methodEntityToSelector(methodEntity)), JavaSourceLoaderImpl.this.getAnnotations(methodEntity), symtab);
      this.parameterTypes = computeParameterTypes(methodEntity);
      this.exceptionTypes = computeExceptionTypes(methodEntity);
    }
  ...
  private class AbstractJavaMethod extends JavaEntityMethod {
    public AbstractJavaMethod(CAstEntity methodEntity, IClass owner, SymbolTable symtab) {
      super(methodEntity, owner, symtab);
    }
  ...
  public class ConcreteJavaMethod extends JavaEntityMethod {
    ...
    public LexicalParent[] getParents() {
      ...
      for (int i = 0; i < parents.length; i++) {
        ...
        IClass c = lookupClass(TypeName.string2TypeName(typeName));
        if(c==null){
          int mark = typeName.lastIndexOf("/");
          typeName = typeName.substring(0, mark)+"$"+typeName.substring(mark+1);
          c = lookupClass(TypeName.string2TypeName(typeName));
        }
        final IClass cls = c;
    ...
    //added by nameng, since sometimes the super method is not recognized
    @Override
    public int getLineNumber(int index) {
        return super.getLineNumber(index);
    }

    @Override
    public IClass getDeclaringClass() {//added by nameng for workaround
...
/** END Custom change: Optional deletion of fTypeMap */
  ...
  public void defineAbstractFunction(CAstEntity n, IClass owner, SymbolTable symtab) {
    ((JavaClass) owner).addMethod(n, owner, symtab);
  }
  ...
  public IClass defineType(CAstEntity type, String typeName, CAstEntity owner) {
    ...
    IClassLoader p = getParent();
    IClass atype = p.lookupClass(javaClass.getName());
    if(atype!=null){
      ArrayList<IClass> cs = new ArrayList<IClass>();
      cs.add(atype);
      p.removeAll(cs);
      atype = p.lookupClass(javaClass.getName());
    }
